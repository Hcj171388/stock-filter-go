...
func main(){
  ...
  for _,it:=range d.Data.Diff{
    for _,k:=range []string{"f12","f2","f3","f62","f21"}{
      if v,ok:=it[k];ok{fmt.Printf("%s => %T : %v\n",k,typeof(v),v)}else{fmt.Printf("%s => MISSING\n",k)}
    }
    fmt.Println("---")
  }
}
func typeof(v interface{}) string { _,ok:=v.(float64); if ok {return "float64"}; s,ok:=v.(string); if ok{return "string"}; return fmt.Sprintf("%T",v) }
